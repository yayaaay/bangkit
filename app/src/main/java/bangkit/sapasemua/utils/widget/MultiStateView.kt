package amirlabs.sapasemua.utils.widget
...
@JvmOverloads constructor(context: Context,
     ...
     * @param drawableRes drawable id of image to display
     * @param title title of error
     * @param message message of error
     * @param btnRetry pair of button text and action when button clicked
     */
    fun showErrorLayout(
        drawableRes: Int? = null,
        title: String? = null,
        message: String? = null,
        btnRetry: Pair<String, () -> Unit>? = null
    ) {
        this.viewState = ViewState.ERROR
        val v = this.getView(ViewState.ERROR)
        drawableRes?.let { v?.findViewById<ImageView>(R.id.ivErrorDefault)?.setImageResource(it) }
        title?.let { v?.findViewById<TextView>(R.id.tvErrorTitleDefault)?.text = it }
        message?.let { v?.findViewById<TextView>(R.id.tvErrorMessageDefault)?.text = it }
        btnRetry?.let { pair ->
            v?.findViewById<Button>(R.id.btnErrorDefault)?.apply {
                visibility = View.VISIBLE
                text = pair.first
    ...
    fun showEmptyLayout(
        ...
        val v = this.getView(ViewState.EMPTY)
        drawableRes?.let { v?.findViewById<ImageView>(R.id.ivEmptyDefault)?.setImageResource(it) }
        message?.let { v?.findViewById<TextView>(R.id.tvEmptyMessageDefault)?.text = it }
    }
}